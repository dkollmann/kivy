...
The :class:`FloatModalView` widget is used to create modal views. By default, the
...
Example of a simple 400x400 Hello world view::
    ...
    view = FloatModalView(size_hint=(None, None), size=(400, 400))
    view.add_widget(Label(text='Hello world'))

By default, any click outside the view will dismiss it. If you don't
want that, you can set :attr:`FloatModalView.auto_dismiss` to False::

    view = FloatModalView(auto_dismiss=False)
    view.add_widget(Label(text='Hello world'))
    view.open()

To manually dismiss/close the view, use the :meth:`FloatModalView.dismiss` method of
the FloatModalView instance::

    view.dismiss()

Both :meth:`FloatModalView.open` and :meth:`FloatModalView.dismiss` are bindable. That
...
True from your callback. ::
    ...
    def my_callback(instance):
        print('FloatModalView', instance, 'is being dismissed, but is prevented!')
        return True
    view = FloatModalView()
    view.add_widget(Label(text='Hello world'))
...
.. versionchanged:: 1.5.0
    The FloatModalView can be closed by hitting the escape key on the
    keyboard if the :attr:`FloatModalView.auto_dismiss` property is True (the
    default).

'''

__all__ = ('FloatModalView', )
...
class FloatModalView(FloatLayout):
    '''FloatModalView class. See module documentation for more information.

    :Events:
        `on_open`:
            Fired when the FloatModalView is opened.
        `on_dismiss`:
            Fired when the FloatModalView is closed. If the callback returns True,
    ...
    def open(self, *largs):
        ...
        if self._window is not None:
            Logger.warning('FloatModalView: you can only open once.')
            return self
        # search window
    ...
    def on_touch_down(self, touch):
        if not self.collide_point(*touch.pos):
            if self.auto_dismiss:
                self.dismiss()
                return True
        super(FloatModalView, self).on_touch_down(touch)
        return True

    def on_touch_move(self, touch):
        super(FloatModalView, self).on_touch_move(touch)
        return True

    def on_touch_up(self, touch):
        super(FloatModalView, self).on_touch_up(touch)
        return True
...
if __name__ == '__main__':
    ...
    view = FloatModalView(size_hint=(None, None), size=(256, 256),
                     auto_dismiss=True)
    view.add_widget(content)
